(function () {
  ...
  // ── スコア計算 ──────────────────────────────────────────────────────────────
  // 6段階リッカート（中立なし）→ 寄与スコア。±3/±4/±5 の離散値。
  // 「どちらかといえば賛成/反対」でも最低 ±3 の重みを持たせることで、
  // バーが中央に寄りすぎる（=結果がぼやける）のを抑える設計。
  // 各軸 8問 × 最大±5 ⇒ 軸合計 sum の理論レンジは ±40。
  var SCORE_BY_OPTION_INDEX = [5, 4, 3, -3, -4, -5];

  // sum===0 の完全同点時に使う「強度重み」係数（|c| → 係数）。
  // 旧スコア配分時の比率（1.0 : 1.2 : 1.5）を踏襲しつつ、IEEE754 の
  // 浮動小数誤差を避けるため整数で表現する（比較関係 >/< のみが意味を持つ）。
  var ZERO_TIE_WEIGHT_BY_ABS_SCORE = { 3: 10, 4: 12, 5: 15 };
  ...
  /**
   * 各軸 8問 × 応答±5 ⇒ 最大|sum|=40 を UI 用 −100〜+100 に線形換算する。
   * 仕様:
