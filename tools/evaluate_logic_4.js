...
for (const code in types) {
  ...
  suited.forEach(p => {
    ...
    if (isGroup && (text.includes("一人で") || text.includes("孤独") || text.includes("黙々と"))) {
      ...
      if (!text.includes("一人で黙々と取り組むより") && !text.includes("一人では出せない") && !text.includes("一人では重い腰が上がらなくても") && !text.includes("一人で行きづらい場所も") && !text.includes("一人でいるときの自分とは別人のように") && !text.includes("一人では挫折しそうな壁を")) {
  ...
  unsuited.forEach(p => {
    ...
    if (isSolo && (text.includes("一人で") || text.includes("孤独"))) {
      if (!text.includes("一人での運動が続かない") && !text.includes("一人でいるときの自分とは別人のように") && !text.includes("一人で自由な環境でこそ") && !text.includes("一人で深く燃えたい") && !text.includes("一人で淡々と進めたい") && !text.includes("一人で己の記録と向き合う")) {
    ...
    if (isGroup && (text.includes("仲間") || text.includes("チーム") || text.includes("みんなで"))) {
      if (!text.includes("仲間との熱量もない") && !text.includes("仲間とのつながりがない") && !text.includes("仲間との絆や") && !text.includes("チームへの不満") && !text.includes("チームの雰囲気が落ちる") && !text.includes("チームの都合に合わせる") && !text.includes("チームが崩れそうなときに") && !text.includes("特定の仲間への約束") && !text.includes("仲間がいれば強く") && !text.includes("共有する仲間も") && !text.includes("チームを勝利に導くプロセスに喜びを感じる") && !text.includes("仲間のために限界を超える") && !text.includes("競争の激しいチーム") && !text.includes("一緒に走る仲間") && !text.includes("チームへの責任感も仲間との熱量もない")) {
