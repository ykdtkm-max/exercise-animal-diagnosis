...
content = content.replace(/concept:\s*['"]([\s\S]*?)['"],/g, (match, p1) => {
  // Only process if it doesn't already have newlines
  if (!p1.includes('\\n')) {
    const textWithNewlines = addNewlines(p1).replace(/\n/g, '\\n');
    return `concept: '${textWithNewlines}',`;
  }
  return match;
});

content = content.replace(/continuation_text:\s*['"]([\s\S]*?)['"],/g, (match, p1) => {
  if (!p1.includes('\\n')) {
    const textWithNewlines = addNewlines(p1).replace(/\n/g, '\\n');
