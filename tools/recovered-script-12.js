...
function padText(text, isSuit) {
  ...
  while (t.length < 95) {
    let needed = 98 - t.length;
    // Find a pad that fits well
    let bestPad = pads[padIdx % pads.length];
    
    // If adding bestPad makes it > 105, we might need a shorter pad
  ...
  // If it's slightly over 105, it's probably fine, but we can try to trim
  if (t.length > 105) {
      // Just leave it if it's close, or we can trim it. 
      // The goal is ~95-105.
  }
