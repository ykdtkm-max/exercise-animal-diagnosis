...
for (const code in types) {
  ...
  unsuited.forEach(p => {
    ...
    if (isGroup && (text.includes("仲間") || text.includes("チーム") || text.includes("みんなで"))) {
      if (!text.includes("仲間との熱量もない") && !text.includes("仲間との絆や") && !text.includes("チームへの不満") && !text.includes("チームの雰囲気が落ちる") && !text.includes("チームの都合に合わせる") && !text.includes("チームが崩れそうなときに") && !text.includes("特定の仲間への約束") && !text.includes("仲間がいれば強く") && !text.includes("共有する仲間も") && !text.includes("チームを勝利に導くプロセスに喜びを感じる") && !text.includes("仲間のために限界を超える")) {
