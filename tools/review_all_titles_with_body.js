...
for (const code in types) {
  ...
  output += "【向いている運動】\n";
  suited.forEach(p => output += `- ${p.short_title}: ${p.body.substring(0, 40)}...\n`);
  output += "【向いていない運動】\n";
  unsuited.forEach(p => output += `- ${p.short_title}: ${p.body.substring(0, 40)}...\n`);
}
fs.writeFileSync('titles_context.txt', output);
console.log("Done");
